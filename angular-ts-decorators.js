import { bootstrap, element, isFunction, module as module$1 } from 'angular';
...
function injectionsKey(property) {
    return property ? metadataKeys.injections + ":" + property : "" + metadataKeys.injections;
}
function addStaticInjections(clz) {
    var injections = getMetadata(injectionsKey(), clz);
...
function Inject(name) {
    return function (target, propertyKey, parameterIndex) {
        ...
        var key = injectionsKey(propertyKey);
        var injections = getMetadata(key, target) || [];
...
var addMethod = function (target, propertyKey, metaKey) {
...
function Run() {
    return function (target, propertyKey) { return addMethod(target, propertyKey, metadataKeys.run); };
}
function Config() {
    return function (target, propertyKey) { return addMethod(target, propertyKey, metadataKeys.config); };
}
function NgModule(_a) {
    ...
    return function (Class) {
        ...
        var runMethods = getMetadata(metadataKeys.run, Class.prototype);
        if (runMethods) {
            runMethods.forEach(function (r) { module.run(r); });
        }
        var configMethods = getMetadata(metadataKeys.config, Class.prototype);
        if (configMethods) {
            configMethods.forEach(function (c) { module.config(c); });
        }
...
export { platformBrowserDynamic, Component, Directive, Injectable, Inject, Pipe, Input, Output, ViewParent, NgModule, Run, Config, HostListener, ViewChild, ViewChildren, ElementRef, ngLifecycleHooksMap, Type, isType, Declaration, metadataKeys, kebabToCamel, camelToKebab, getAttributeName, isAttributeSelector, getMetadata, defineMetadata, getTypeName, getTypeDeclaration, injectionsKey, addStaticInjections };
